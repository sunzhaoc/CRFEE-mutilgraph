            ...
            for z = 1:test_num
                for i = 1:10
                    ...
                    for j = 1:min_dim
                        ...
                        for k = 0:graph_num
                            load (['./Processed_dataset/', datasetname, '/03PCA_2/test/split', num2str(i), '/', num2str(graphchoose(:, k+1))])
                            load (['./Processed_dataset/', datasetname, '/03PCA_2/train/split', num2str(i), '/', num2str(graphchoose(:, k+1))])
                            load (['./Processed_dataset/', datasetname, '/05CRFEE_P_2/train/split', num2str(i), '/', num2str(graphchoose(:, k+1))]);
                ...
                filepath = (['./Result_2/', datasetname, '/']);
                if ~exist(filepath, 'dir')
                    mkdir(filepath)
                end
                save([filepath, [num2str(graph_num+1)]], 'final_acc');
            end
            clearvars -except choosedata graph_num
        end
        clearvars -except graph_num
    end
end
